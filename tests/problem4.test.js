/*
    ...
    takeFirst(3, p1, p2, p3, p4, r1).then(console.log) // [p3, p2, p1]
...
describe('problem4', () => {
  it.only('resolves with an array of as many elements as first argument specifies', async () => {
  ...
  it.only('resolves with an empty array if no promises have been passed', async () => {
    expect(await takeFirst(3)).toEqual([]);
  });

  it.only('resolves with given number of first resolved promises', async () => {
  ...
  it("doesn't wait for all Promises to resolve/reject", async () => {
    ...
    expect(await takeFirst(2, promise1, promise2, promise3)).toEqual([5, 7]);
  });
});
